			...
			<div class="container">
		 		<section  class="  center" v-for="topic in topics">
					...
					<p style="font-weight: 300; opacity: .7; color: black;  ">
							Review topic for <a :href="'/c/' +  categorytype + '/' + topic.user_code">@{{ topic.name }}</a> on @{{ topic.created_at }} </p> <span v-if="topic.user_name"> by <a target="_blank" :href="'/p/' + topic.user_code">@{{topic.user_name}}</a></span>
